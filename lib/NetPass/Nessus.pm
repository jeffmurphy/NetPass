...
NetPass::Nessus - integration with Nessus

=head1 SYNOPSIS

    use NetPass::Nessus qw(_log _cont);
...
This module is largely Net::Nessus::ScanLite. We've modified it to have
a showSTATUS() routine as well as callback functionality. This is used
by the webserver to provide a progress bar. Other than that, props to the
author of ScanLite (and the author of Nessus::Client and Nessus::Message 
upon which ScanLite is built).

=head1 MODIFICATIONS BY

Jeff Murphy <jcmurphy@buffalo.edu>

=head1 CREDITS/AUTHOR

John Ballem, <jpb@brown.edu>  (Net::Nessus::ScanLite)

=head1 REVISION

$Id: Nessus.pm,v 1.1 2004/09/24 01:05:20 jeffmurphy Exp $

=cut


package NetPass::Nessus;
...
sub ShowSTATUS {
        my ($this,$msg) = @_;
        my $key = '_status';
	_log("DEBUG", "msg=".join(' ', @$msg)."\n") if $this->{'debug'};

	if (defined($this->{'callback'})) {
		&{$this->{'callback'}} ($msg, $this->{'callbackparms'});
	}
